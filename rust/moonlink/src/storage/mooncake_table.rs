...
mod tests {
    ...
    #[tokio::test]
    async fn test_deletion_remap_in_memslice() -> Result<()> {
        // Create a temporary directory for test data
        let temp_dir = tempdir().unwrap();
        let test_dir = temp_dir.path().join("test_deletions_dir");
        create_dir_all(&test_dir).unwrap();
        ...
        // Delete some rows before flush but dont commit
        let delete_rows = vec![MoonlinkRow::new(vec![
            RowValue::Int32(2),
            RowValue::ByteArray("Row 2".as_bytes().to_vec()),
            RowValue::Int32(32),
        ])];
        ...
        // Create snapshot before flush to verify deletions are tracked
        // Note that they will be added to uncommitted_deletion_log
        let snapshot_handle = table.create_snapshot().unwrap();
        assert!(
            snapshot_handle.await.is_ok(),
            "Snapshot before flush failed"
        ...
        // Flush the records that the deletions point to
        let flush_handle = table.flush(2);
        let disk_slice = flush_handle.await.unwrap()?;
        table.commit_flush(disk_slice)?;

        // Create snapshot
        // In this case we will attempt to remap recrods in uncommitted_deletion_log
        // Since these point to batches that were flushed, they will be flagged for remap
        // But since we are deleting a record from the batch itself it never made it to disk
        // So there is actually no location stored in new_location
        // In this case we can simply skip remap
        let snapshot_handle = table.create_snapshot().unwrap();
        assert!(
            snapshot_handle.await.is_ok(),
            "Final snapshot creation failed"
